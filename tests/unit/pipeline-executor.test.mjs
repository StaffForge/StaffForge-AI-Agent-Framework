...
async function runTests() {
  ...
  {
    const result = await executor.execute('feature', 'implement user auth');
  ...
  {
    const result = await executor.execute('bugfix', 'fix login crash');
  ...
  {
    const result = await executor.execute('unknown', 'test');
  ...
  {
    const result = await executor.execute('feature', 'build rest api');
  ...
  {
    const result = await executor.execute('security', 'audit auth tokens');
  ...
  // Test 6: All pipeline types get a modelProfile
  {
    for (const type of ['feature', 'bugfix', 'refactor', 'security', 'deployment', 'hotfix']) {
      const result = await executor.execute(type, 'test');
  ...
  {
    const result = await executor.execute('bugfix', 'fix docker networking issue');
